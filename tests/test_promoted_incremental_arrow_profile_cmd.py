...
def test_run_profiles_arrow_only_incremental_path(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert payload["seed_source"] == "clusters"
    ...
    assert payload["summary"]["memory_estimates"]["candidate_row_count"] == {"min": 2.0, "p50": 2.0, "max": 2.0}
...
def test_run_uses_synthetic_profile_seeds_when_clusters_missing(
    ...
    rows = [
        cmd.ArrowSignatureRow("a", "pa", "large", "Ada", "", "Lovelace", None),
        cmd.ArrowSignatureRow("b", "pb", "large", "Grace", "", "Hopper", None),
        cmd.ArrowSignatureRow("c", "pc", "large", "Katherine", "", "Johnson", None),
        cmd.ArrowSignatureRow("d", "pd", "large", "Hedy", "", "Lamarr", None),
    ]
    ...
    class FakeClusterer:
        ...
        def predict_incremental(
            ...
            **_kwargs: Any,
            ...
            return {
                "clusters": {"synthetic": block_signatures},
    ...
    assert payload["seed_source"] == "synthetic"
    assert payload["seed_signature_count"] == 2
    assert captured_calls[0]["block_signatures"] == ["a", "b", "c", "d"]
    assert "clusters" not in captured_calls[0]["arrow_paths"]
    assert written_seed_maps == {
        Path(captured_calls[0]["arrow_paths"]["cluster_seeds"]): {
            "a": "synthetic:large:0",
            "b": "synthetic:large:1",
        }
    }
...
def test_run_requires_seed_source_when_clusters_missing(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.setattr(
        cmd,
        "_resolve_arrow_dataset_paths",
        lambda _root, _dataset: {"signatures": str(tmp_path / "signatures.arrow")},
    )
    monkeypatch.setattr(
        cmd,
        "_read_signature_rows",
        lambda _path: [cmd.ArrowSignatureRow("a", "pa", "large", "Ada", "", "Lovelace", None)],
    )
    args = cmd.parse_args(["--arrow-root", str(tmp_path), "--dataset", "dummy"])

    with pytest.raises(FileNotFoundError, match="--synthetic-seeds-when-clusters-missing"):
        cmd.run(args)
